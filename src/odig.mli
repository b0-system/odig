...
(** Mining OCaml package installs.
    ...
    See the {{!tutorial}tutorial} and the {{!conv}packaging conventions}.
...
(** {1:tutorial Tutorial}
     ...
     packages. The list of packages recognized by odig is returned by
{v
odig pkg
v}

    [odig] provides a few commands that make it easy to lookup
    the readme, change log and license files of packages (here
    applied on the [odig] package itself):
{v
odig readme odig
odig changes odig
odig license odig
v}
    If the package properly installed its opam metadata file you can
    also quickly get access to the issues, homepage, or online
    documentation pages of a package. The following commands opens
    them in your browser:
{v
odig issues odig
odig homepage odig
odig online-doc odig
v}
     See the help of individual commands for more details and options.
     Package API documentation can be generated on a best-effort basis
     with odoc by issuing:
{v
odig odoc       # Generate API docs for all packages
odig odoc odig  # Generate API docs for the odig package
v}
    To open the documentation index or the documentation of
    a package in your browser use:
{v
odig doc      # Package index
odig doc odig  # API doc for package odig
v} *)

(** {1:conv Packaging conventions}

    Packages following these convention maximize odig's mining
    capabilities.

    {2:conv_exist Package install structure and existence}

    [odig] assumes all OCaml packages are installed in a library
    prefix called [LIBDIR] and have their distribution documentation
    installed in [DOCDIR].

    For a package named [PKG] to be recognized by odig, [PKG] must be a
    valid opam package name and one of the following paths must exist:

{v
LIBDIR/PKG/opam
LIBDIR/PKG/META
LIBDIR/PKG/caml (deprecated)
v}

    If neither exists [LIBDIR/PKG] is ignored by odig and the package
    [PKG] does not exist. For each package [PKG] its distribution
    documentation (readme, license, change log, sample code, etc.) is
    looked up in the directory [DOCDIR/PKG].

    Typically the values of [LIBDIR] and [DOCDIR] are
    [opam var lib] and [opam var doc]. However odig is not tied to [opam],
    the only assumption made by [odig] is above install structure is
    followed.

    {2:meta Metadata detection}

    Package metadata for [PKG] is always read from [LIBDIR/PKG/opam]
    which must be a valid opam file. If present, the following fields
    are consulted and used by odig in various context and/or commands.

    {ul
    {- [authors:], the authors, [authors] command}
    {- [bug-reports:], the issue tracker URI, [issues] command}
    {- [deps:, depopts:], the dependencies, [deps] command}
    {- [dev-repo:], the source repository URI, [repo] command}
    {- [doc:], the online documentation URI, [online-doc] command}
    {- [homepage:], the homepage URI, [homepage] command}
    {- [license:], license tags, [license -t] command}
    {- [maintainers:], the maintainers, [maintainers] command}
    {- [tags:], classification tags, [tags] command}
    {- [version:], the version string, [version] command}}

    {2:distribdoc Distribution documentation detection}

    The distribution documentation for a package [PKG] is determined
    by {e caseless} matching files in [DOCDIR/PKG]. The following
    matches are performed; multiple files are allowed to match and *
    denotes zero or more characters.

    {ul
    {- [DOCDIR/PKG/README*], readmes, [readme] command}
    {- [DOCDIR/PKG/CHANGE*] or [DOCDIR/PKG/HISTORY*]
       change logs, [changes] command}
    {- [DOCDIR/PKG/LICENSE*], license, [license] command}}

    {2:api OCaml API detection and doc generation}

    The OCaml API of a package [PKG] is defined by all the compiled
    module interfaces (cmi) files that are present in the path
    hierarchy rooted at [LIBDIR/PKG].

    The odoc API documentation of a package [PKG] is generated by
    considering all cmi files of the API, the files in
    [DOCDIR/PKG/odoc-assets] and the files in [DOCIDIR/PKG/odoc-pages]

    For each cmi file if a corresponding cmti file is found at
    ...
    Any file in [DOCDIR/PKG/odoc-assets] is copied over
    in an [_assets] directory at the root of the package's generated
    API documentation.

    {b FIXME.}
    {ul
    {- Say something about multiple cmi files with the same name.}
    {- Explain that undigested dependency names are only resolved in the
       same package.}}
*)
